...
public class UsuarioDAOArchivo implements UsuarioDAO {
    ...
    private static final int TAM_REGISTRO= (TAM_CEDULA * 2)+ (TAM_NOMBRE * 2)+ (TAM_APELLIDO * 2)+ (TAM_TELEFONO * 2)+ (TAM_CORREO * 2)+ (TAM_DIRECCION * 2)+ 8 + 1;
    ...
    private void escribirUsuario(RandomAccessFile archivo, Usuario usuario) throws IOException {
        ...
        archivo.writeLong(usuario.getFechaNacimiento().getTime());
    ...
    public Usuario buscar(String cedula) throws UsuarioNoExiste {
        ...
        try (RandomAccessFile archivo = new RandomAccessFile(ruta, "r")) {
            ...
            throw new UsuarioNoExiste("No se encontro el archivo de usuario");
        }
        if(encontrado == null){
            throw new UsuarioNoExiste("No se encontro usuario" + cedula);
        }
        return encontrado;
    }
